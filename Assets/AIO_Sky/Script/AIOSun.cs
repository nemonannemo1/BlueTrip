...
/// </summary>
        ...
        //v1.2  
        public float sunIntensity=1.0f; //wip multiply intensity presented
        [Range(0f, 24f)]
    ...
    remove in v1.3 */
        ...
        // private
        [HideInInspector]
        //remove in v1.3// public Material skyBoxEditor;
        // remove in v1.3// static Material skyBoxRuntime;
        ...
        //remove in v1.3*/
        //remove in v1.3//[Range(-0.5f, 0.85f)]
        //remove in v1.3//public float groundLevel;
        //remove in v1.3//Color aioGroundColor;

        bool setFog = false;
        Color oldFogColor, oldGroundColor, oldAioFog;
        //remove in v1.3// float oldGroundLevel, oldFogLevel;
        ...
        private void OnEnable()
        {
            ...
            /*//remove in v1.3/// hack between runtime & editor setting
        ...
        private void LateUpdate()
        {
            ...
            //v1.3 to do check editor change skybox, copy material runtime to protest it
            ...
            else
            {
                ...
                {
                    Vector3 lightForward = Vector3.Normalize(sunTransform.forward);
                    ////remove in v1.3 sunLight.intensity = Mathf.SmoothStep(sunIntensity, 0, 2 * (Mathf.Clamp(lightForward.y, nightRange, setRange) - nightRange) / (setRange - nightRange));
                    if (sunAnimation)
                    {
                        SetAIOTime(sunHour);
...
#if UNITY_EDITOR
          //  if (AIOskyBox != RenderSettings.skybox) { AIOskyBox = RenderSettings.skybox;}
            ...
            if (!Application.isPlaying)
            {
                ...
                if (sunShaftsEnable)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Destroy(aioSSHelper.gameObject);
                            CreateSSHelper();
                            aioSSHelper.enabled = true;
                        }
                        else
        ...
        private void OnDisable()
        {
            ...
            //remove in v1.3 //RenderSettings.skybox = skyBoxEditor;
            //v1.2
            //*//remove in v1.3
            if (sunLight != null)
            {
        ...
        {
            Material matTemp = RenderSettings.skybox;
            if (mat.shader != matTemp.shader)//only same shader can lerp
            {
        ...
        {
            if (mat.shader != RenderSettings.skybox.shader)//only same shader can lerp
            {
                yield break;
            }
           ...
           // matTemp.SetFloat("_moonScale", 1 / moonScale);
            ...
            do
            {
                ...
                RenderSettings.skybox.Lerp(RenderSettings.skybox, matTemp, Mathf.Pow(localTime, gamma) * .5f);
