...
Shader "AIOsky/Half01"
{
	Properties
	{
		...
		_BGscale("BGscale", Range( 0.2 , 1)) = 0.418
	...
	SubShader
	{
		...
		uniform half _setRange;
		uniform half _DayRange;
		uniform half _sunMax;
		uniform half _sunMin;
		uniform half4 _SunSetSky;
		uniform half4 _DaySky;
		uniform half4 _NightSky;
		uniform half _nightRange;
		uniform sampler2D _Moon;
		uniform half3 _MoonPosition;
		uniform half _moonScale;
		uniform sampler2D _StarField;
		uniform half _BGscale;
		uniform half _BGRotate;
		uniform half4 _SunSetAtmosphere;
		uniform half4 _DayAtmosphere;
		uniform half4 _NightAtmosphere;
		uniform half _AtmosphereStart;
		uniform half _AtmosphereEnd;
		uniform half _SunGlow;
		uniform half4 _ExtraLight;
		uniform sampler2D _MoonGlow;
		uniform fixed _ToggleExtra;
		uniform sampler2D _BaseCloud;
		uniform half _CloudsRotation;
		uniform half _CloudsScale;
		uniform half _DomeCurved;
		uniform half2 _CloudsPan;
		uniform half _timeScale;
		uniform sampler2D _DistortionTexture;
		uniform half _DistortionScale;
		uniform half2 _DistortionPan;
		uniform half _DistortionTime;
		uniform half _DistortionMax;
		uniform half _DistortionMin;
		uniform half _CloudsDensity;
		uniform half _CloudsThickness;
		uniform half _CloudsTransmission;
		uniform half _CloudsShadowWeight;
		uniform half _CloudsShadowBalance;
		uniform half _remapswitch;
		uniform fixed _normalMapenable;
		uniform sampler2D _normalTexture;
		uniform half _normalscale;
		uniform half _YScale;
		uniform half _DotOffset;
		uniform half _CloudsBrightness;
		uniform half _CloudsContract;
		uniform half4 _FogColor;
		uniform half4 _GroundColor;
		uniform half _GroundLevel;
		uniform half _FogLevel;
		...
		void surf( Input i , inout SurfaceOutput o )
		{
			...
			half4 tex2DNode434 = tex2D( _Moon, clamp( ( appendResult425 + ( temp_output_422_0 + float2( 0.5,0.5 ) ) ) , float2( 0,0 ) , float2( 1,1 ) ) );
			...
			half4 tex2DNode495 = tex2D( _DistortionTexture, ( ( ( ( ( rotator87_g42 * localMyCustomExpression62_g4262_g42 ) + ( _CloudsPan + mulTime359 ) ) * _DistortionScale ) + _DistortionPan ) + mulTime514 ) );
			...
			float componentMask596 = tex2D( _BaseCloud, temp_output_526_0 ).x;
