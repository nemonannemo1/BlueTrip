...
Shader "AIOsky/V2"
{
	...
	SubShader
	{
		...
		inline float MyCustomExpression62_g226( float InY , float InS , float InH )
		{
		...
		void surf( Input i , inout SurfaceOutput o )
		{
			...
			float clampResult2_g394 = clamp( (0.0 + (ViewDirY797 - 0.0) * (1.0 - 0.0) / (_GroundLevel - 0.0)) , 0.0 , 1.0 );
			float4 lerpResult633 = lerp( _FogColor , _GroundColor , clampResult2_g394);
			float clampResult2_g396 = clamp( (0.0 + (ViewDirY797 - ( _GroundLevel - _FogLevel )) * (1.0 - 0.0) / (_GroundLevel - ( _GroundLevel - _FogLevel ))) , 0.0 , 1.0 );
			float temp_output_3_0_g397 = clampResult2_g396;
			float3 break100_g226 = ViewDir1014;
			float2 appendResult86_g226 = (float2(break100_g226.x , break100_g226.z));
			float cos87_g226 = cos( _CloudsRotation );
			float sin87_g226 = sin( _CloudsRotation );
			float2 rotator87_g226 = mul( appendResult86_g226 - float2( 0,0 ) , float2x2( cos87_g226 , -sin87_g226 , sin87_g226 , cos87_g226 )) + float2( 0,0 );
			float InY62_g226 = break100_g226.y;
			float temp_output_606_0 = ( _CloudsScale * 0.0001 );
			float temp_output_24_0_g226 = _DomeCurved;
			float InS62_g226 = ( temp_output_606_0 / max( ( break100_g226.y - temp_output_24_0_g226 ) , 1E-05 ) );
			float InH62_g226 = temp_output_24_0_g226;
			float localMyCustomExpression62_g226 = MyCustomExpression62_g226( InY62_g226 , InS62_g226 , InH62_g226 );
			...
			float4 temp_output_772_0 = ( tex2D( _BaseCloud, ( ( ( rotator87_g226 * localMyCustomExpression62_g226 ) + float2( 0,0 ) ) + temp_output_804_0 ) ) + ( tex2D( _CloudDetail, ( ( ( rotator87_g172 * localMyCustomExpression62_g172 ) + float2( 0,0 ) ) + temp_output_804_0 + appendResult940 ) ) * 0.5 ) );
			...
			float clampResult2_g364 = clamp( (0.0 + (normalizeResult1074.y - _setRange) * (1.0 - 0.0) / (_nightRange - _setRange)) , 0.0 , 1.0 );
			float nightControl484 = clampResult2_g364;
			...
			float4 lerpResult336 = lerp( lerpResult329 , _NightAtmosphere , clampResult2_g364);
			...
			float temp_output_7_0_g391 = _CloudsBrightness;
			float4 temp_cast_9 = (temp_output_7_0_g391).xxxx;
			float temp_output_4_0_g391 = ( temp_output_7_0_g391 + _CloudsContract );
			float4 temp_cast_10 = (temp_output_4_0_g391).xxxx;
			float4 clampResult2_g391 = clamp( clampResult951 , temp_cast_9 , temp_cast_10 );
			float4 temp_cast_11 = (temp_output_7_0_g391).xxxx;
			float4 temp_cast_12 = (temp_output_4_0_g391).xxxx;
			float4 lerpResult905 = lerp( (float4( 0,0,0,0 ) + (clampResult2_g392 - temp_cast_7) * (float4( 1,1,1,1 ) - float4( 0,0,0,0 )) / (temp_cast_8 - temp_cast_7)) , (float4( 0,0,0,0 ) + (clampResult2_g391 - temp_cast_11) * (float4( 1,1,1,1 ) - float4( 0,0,0,0 )) / (temp_cast_12 - temp_cast_11)) , DayControl901);
			...
			float temp_output_7_0_g393 = _CloudsDensity;
			float temp_output_4_0_g393 = ( temp_output_7_0_g393 + _CloudsThickness );
			float clampResult2_g393 = clamp( min( clampResult1038 , step( break100_g226.y , temp_output_24_0_g226 ) ) , temp_output_7_0_g393 , temp_output_4_0_g393 );
			float temp_output_3_0_g395 = (0.0 + (clampResult2_g393 - temp_output_7_0_g393) * (1.0 - 0.0) / (temp_output_4_0_g393 - temp_output_7_0_g393));
			float3 ViewDirNormal803 = ViewDir1014;
			float dotResult309 = dot( normalizeResult1074 , -ViewDirNormal803 );
			float clampResult2_g366 = clamp( (0.0 + (dotResult309 - ( _sunMax - _sunMin )) * (1.0 - 0.0) / (_sunMax - ( _sunMax - _sunMin ))) , 0.0 , 1.0 );
			float temp_output_800_0 = clampResult2_g366;
			...
			float4 lerpResult335 = lerp( lerpResult327 , _NightSky , clampResult2_g364);
			...
			float4 lerpResult439 = lerp( ( lerpResult335 + ( tex2DNode434 * 0.1 * tex2DNode434.a ) ) , clampResult985 , clampResult2_g364);
			...
			o.Emission = lerp(( ( lerpResult633.rgb * temp_output_3_0_g397 ) + ( ( ( clampResult946.rgb * temp_output_3_0_g395 ) + ( SkyOnly342.rgb * ( 1.0 - temp_output_3_0_g395 ) ) ) * ( 1.0 - temp_output_3_0_g397 ) ) ),temp_cast_15,_ToggleTest);
			o.Alpha = 1;
		}
